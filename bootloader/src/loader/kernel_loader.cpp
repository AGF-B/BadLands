...
using namespace ExecutableFormat;
namespace ShdMem = Shared::Memory;

namespace {
    [[noreturn]] static inline constexpr void notify_corrupted_kernel(unsigned int code) {
    ...
    static inline KernelLocInfo load_kernel_pe(
        ...
        else if (elf_hdr->Format != ELF_64
                || elf_hdr->Endianness != ELF_LE
                || elf_hdr->MArch != ELF_MARCH_x86_64
        ) {
        ...
        const ELF_PROGRAM_HEADER* elf_phdr_table = reinterpret_cast<const ELF_PROGRAM_HEADER*>(
            kernel_data + elf_hdr->ProgramHeaderTableOffset
        );

        for (size_t i = 0; i < elf_hdr->ProgramHeadersCount; ++i) {
            if (elf_phdr_table[i].SegmentType == ELF_LOAD_SEGMENT) {
                size_t segment_pages =
                    (elf_phdr_table[i].SegmentMemorySize + ShdMem::PAGE_SIZE - 1) / ShdMem::PAGE_SIZE;
                ...
                EFI::sys->BootServices->SetMem(
                    reinterpret_cast<VOID*>(segment_memory),
                    segment_pages * ShdMem::PAGE_SIZE,
                    0
                );
